...
#define ProductCodeBeckhoffEL6001 0x17713052
...
#define EL6001_PORT_NAME_SIZE 16

typedef struct {
    ec_tty_t *tty;
	char name[EL6001_PORT_NAME_SIZE];
...
#define EL6001_PORTS 1

typedef struct {
    struct list_head list;
    ec_slave_config_t *sc;
	el60xx_port_t port[EL6001_PORTS];
} el6001_t;
 ...
 * Product code:    0x17713052
 * Revision number: 0x00100000
 */

ec_pdo_entry_info_t el6001_pdo_entries[] = {
   {0x7000, 0x01, 16}, /* Ctrl */
...
ec_pdo_info_t el6001_pdos[] = {
   {0x1604, 23, el6001_pdo_entries + 0}, /* COM RxPDO-Map Outputs Ch.1 */
   {0x1a04, 23, el6001_pdo_entries + 23}, /* COM TxPDO-Map Inputs Ch.1 */

};

ec_sync_info_t el6001_syncs[] = {
   {0, EC_DIR_OUTPUT, 0, NULL, EC_WD_DISABLE},
   {1, EC_DIR_INPUT, 0, NULL, EC_WD_DISABLE},
   {2, EC_DIR_OUTPUT, 1, el6001_pdos + 0, EC_WD_DISABLE},
   {3, EC_DIR_INPUT, 1, el6001_pdos + 1, EC_WD_DISABLE},
   {0xff}
};
        ...
        ec_domain_t *domain, unsigned int slot_offset, const char *name)
{
   ...
   strncpy(port->name, name, EL6001_PORT_NAME_SIZE);
    ...
    ret = ecrt_slave_config_reg_pdo_entry(
            sc, 0x7000 + slot_offset, 0x01, domain, NULL);
    if (ret < 0) {
    ...
    ret = ecrt_slave_config_reg_pdo_entry(
            sc, 0x6000 + slot_offset, 0x01, domain, NULL);
    if (ret < 0) {
        printk(KERN_ERR PFX "Failed to register PDO entry of %s.\n",
                port->name);
        goto out_free_tty;
...
int el6001_init(el6001_t *el6001, ec_master_t *master, u16 position,
        ec_domain_t *domain, u32 vendor, u32 product)
{
	
    int ret = 0, i;

    if (!(el6001->sc = ecrt_master_slave_config(
                    master, 0, position, vendor, product))) {
        printk(KERN_ERR PFX "EL6001(%u): Failed to create"
                " slave configuration.\n", position);
        ret = -EBUSY;
        goto out_return;
    }
    
    if (ecrt_slave_config_pdos(el6001->sc, EC_END, el6001_syncs)) {
        printk(KERN_ERR PFX "EL6001(%u): Failed to configure PDOs.\n",
    ...
    for (i = 0; i < EL6001_PORTS; i++) {
        char name[EL6001_PORT_NAME_SIZE];
        snprintf(name, EL6001_PORT_NAME_SIZE, "EL6002(%u) X%u",
                position, i + 1);
        if (el60xx_port_init(el6001->port + i, el6001->sc, domain, i * 0x10,
                    name)) {
            printk(KERN_ERR PFX "EL6001(%u): Failed to init port X%u.\n",
                    position, i);
            goto out_ports;
        }
    }
...
out_ports:
    for (i--; i >= 0; i--) {
        el60xx_port_clear(el6001->port + i);
    }
out_return:
    return ret;
}
...
/****************************************************************************/
void el6001_clear(el6001_t *el6001)
{
    int i;
...
void el6001_run(el6001_t *el6001, u8 *pd)
{
    int i;

    for (i = 0; i < EL6001_PORTS; i++) {
        el60xx_port_run(el6001->port + i, pd);
    }
}
...
void run_serial_devices(u8 *pd)
{
    ...
    list_for_each_entry(el6001, &handlers, list) {
        el6001_run(el6001, pd);
    }
}

/*****************************************************************************/

int create_el6001_handler(ec_master_t *master, ec_domain_t *domain,
        u16 position, u32 vendor, u32 product)
{
    el6001_t *el6001;
    int ret;

    printk(KERN_INFO PFX "Creating handler for EL6001 at position %u\n",
            position);

    el6001 = kmalloc(sizeof(*el6001), GFP_KERNEL);
    if (!el6001) {
        printk(KERN_ERR PFX "Failed to allocate serial device object.\n");
        return -ENOMEM;
    }

    ret = el6001_init(el6001, master, position, domain, vendor, product);
    if (ret) {
        kfree(el6001);
        return ret;
    }

    list_add_tail(&el6001->list, &handlers);
    return 0;
}
...
int create_serial_devices(ec_master_t *master, ec_domain_t *domain)
{
    ...
    for (i = 0; i < master_info.slave_count; i++) {
        ...
        if (slave_info.vendor_id == VendorIdBeckhoff
                && slave_info.product_code == ProductCodeBeckhoffEL6001) {
            if (create_el6001_handler(master, domain, i,
